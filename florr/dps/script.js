...
//Rotation Buttons.
ID("nrotate").onclick = function () {
  ID("rotate").value = 2.5;
};
ID("crotate").onclick = function () {
  ID("rotate").value = 2.8;
};
ID("urotate").onclick = function () {
  ID("rotate").value = 3.1;
};
ID("rrotate").onclick = function () {
  ID("rotate").value = 3.4;
};
ID("erotate").onclick = function () {
  ID("rotate").value = 3.7;
};
ID("lrotate").onclick = function () {
  ID("rotate").value = 4;
};
ID("mrotate").onclick = function () {
  ID("rotate").value = 4.3;
};
ID("ulrotate").onclick = function () {
  ID("rotate").value = 4.6;
};
ID("srotate").onclick = function () {
  ID("rotate").value = 4.9;
};
ID("cerotate").onclick = function () {
  ID("rotate").value = 5.2;
};
ID("chrotate").onclick = function () {
  ID("rotate").value = 5.5;
};
ID("efrotate").onclick = function () {
  ID("rotate").value = 5.8;
};

//Reload Buttons.
ID("nreload").onclick = function () {
  ID("reload").value = 1;
};
ID("creload").onclick = function () {
  ID("reload").value = 0.93;
};
ID("ureload").onclick = function () {
  ID("reload").value = 0.86;
};
ID("rreload").onclick = function () {
  ID("reload").value = 0.8;
};
ID("ereload").onclick = function () {
  ID("reload").value = 0.75;
};
ID("lreload").onclick = function () {
  ID("reload").value = 0.69;
};
ID("mreload").onclick = function () {
  ID("reload").value = 0.64;
};
ID("ulreload").onclick = function () {
  ID("reload").value = 0.6;
};
ID("sreload").onclick = function () {
  ID("reload").value = 0.54;
};
ID("cereload").onclick = function () {
  ID("reload").value = 0.49;
};
ID("chreload").onclick = function () {
  ID("reload").value = 0.45;
};
ID("efreload").onclick = function () {
  ID("reload").value = 0.39;
};
...
ID("runbutton").onclick = function () {
  ...
  let petrot = Math.ceil(
    (ID("rotate").value * (petrld + 0.04 * (mobstats - 1))) / (2 * Math.PI)
  );
  //Final
  let total =
    (mobstats * (ID("petdmg").value * ID("petnum").value)) /
    ((2 * petrot * Math.PI) / ID("rotate").value);

  //Submit info to the website

  //ROTATION RADIUS.
  function rotateResultID(num) {
    ID("rotateresult").innerHTML = `ROTATION RADIUS: <span style="color:#${
      colorRarities[num]
    }">${ID("rotate").value} rad/s</span>`;
  ...
  function reloadResultID(num) {
    ID("reloadresult").innerHTML = `RELOAD REDUCTION: <span style="color:#${
      colorRarities[num]
    }">x${ID("reload").value}</span>`;
  ...
  function petalUsedID(num) {
    ID("petalused").innerHTML = `PETAL USED: <span style="color:#${
      colorRarities[num]
    }">${ID("petnum").value} ${ID("petname").value}</span>`;
  ...
  function mobUsedID(num) {
    ID("mobused").innerHTML = `MOB ATTACK: <span style="color:#${
      colorRarities[num]
    }">${ID("mobname").value}</span>`;
  ...
  function reloadReductID(num) {
    ID(
      "reloadreduct"
    ).innerHTML = `PETAL COOLDOWN: <span style="color:#${colorRarities[num]}">${petrld}s</span>`;
  ...
  function mobStatsID(num) {
    ID("mobstats").innerHTML = `MOB STATS: <span style="color:#${
      colorRarities[num]
    }">${numComma(mobstats)}</span>`;
  ...
  if (mobstats < 250) {
    mobStatsID(0);
  } else if (mobstats <= 500) {
    mobStatsID(1);
  } else if (mobstats <= 750) {
    mobStatsID(2);
  } else if (mobstats <= 1000) {
    mobStatsID(3);
  } else if (mobstats <= 5000) {
    mobStatsID(4);
  } else if (mobstats <= 10000) {
    mobStatsID(5);
  } else if (mobstats <= 15000) {
    mobStatsID(6);
  } else if (mobstats <= 20000) {
    mobStatsID(7);
  } else if (mobstats <= 27500) {
    mobStatsID(8);
  } else if (mobstats <= 35000) {
    mobStatsID(9);
  } else if (mobstats > 35000) {
    mobStatsID(10);
  }
  ...
  function hitsRotationID(num) {
    ID("petalrotate").innerHTML = `HITS EACH ROTATION: <span style="color:#${
      colorRarities[num]
    }">${numComma(petrot)}</span>`;
  ...
  //TOTAL DPS
  ID("total1").innerHTML =
    numComma(ID("petnum").value) + " " + ID("petname").value;
  ...
  //Show / Hide Formulas
  let isFormula = false,
    classFormula = document.getElementsByClassName("formula");
